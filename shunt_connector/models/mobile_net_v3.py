# -*- coding: utf-8 -*-
"""
Implements the MobileNetV3 feature extractor for classification and segmentation and tasks.
...
def create_mobilenet_v3(input_shape=(32,32,3),
                        num_classes=10,
    ...
    if is_pretrained_on_imagenet:
        ...
        if is_small:
            mobilenet = MobileNetV3Small(weights='imagenet',
                                         include_top=True,
                                         alpha=depth_factor,
                                         classes=num_classes,
        ...
        if is_small:
            mobilenet = MobileNetV3Small(weights=None,
                                         include_top=True,
                                         alpha=depth_factor,
                                         classes=num_classes,
                                         input_shape=input_shape,
            ...
            mobilenet = MobileNetV3Large(weights=None,
                                         include_top=True,
                                         alpha=depth_factor,
                                         classes=num_classes,
                                         input_shape=input_shape,
...
# ORIGINAL keras_application's MobileNetV3 with slight modifications.
...
def hard_sigmoid(x):
    return layers.ReLU(6.)(x + 3.) * 0.16667
def hard_swish(x):
    return layers.Multiply()([layers.ReLU(6.)(x + 3.) * (1./6.), x])
...
def _se_block(inputs, filters, se_ratio, prefix):
    ...
    if backend.image_data_format() == 'channels_first':
        x = layers.Reshape((filters, 1, 1), name=prefix + 'reshape')(x)
    else:
        x = layers.Reshape((1, 1, filters), name=prefix + 'reshape')(x)
    ...
    x = layers.Activation(hard_sigmoid, name=prefix + 'activation')(x)
...
def _inverted_res_block(x, expansion, filters, kernel_size, stride,
                        se_ratio, activation, block_id, rate=1, return_expand_tensor=False, use_skip_connection=True):
    ...
    if stride == 1 and infilters == filters and use_skip_connection:
        x = layers.Add(name=prefix + 'Add')([shortcut, x])

    if return_expand_tensor:
        return x, expanded_tensor
    else:
        return x
